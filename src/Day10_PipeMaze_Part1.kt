...
|-|-J.--LLLF|JJ.FFLJL-J-JJL--JF-L7-|J.F.LJ.L-J|L|77--J|-J.JL|.J.LJJL-JJLJJ.-J-J-7-7-LJL7L|-L-JJLLFJF--L|J--JFJF-7-LLJL--|---F.--|7J.L-L7JJ-.
    ...
    for (pipeAdjTo in pipesAdjToS) {
        steps = 1
        var currentPipe = getNextPipe(pipeAdjTo, pipes, previousPipe)
        previousPipe = pipeAdjTo
        while(currentPipe != null && currentPipe.xy  !=  sPipe.xy){
            steps++;
            var nextPipe = getNextPipe(currentPipe, pipes, previousPipe)
            previousPipe = currentPipe
            currentPipe = nextPipe
        }

        if(currentPipe?.xy  ==  sPipe.xy){
            steps++
            break
        }
...
fun extractPipes(lines: Array<String>): MutableList<Pipe> {
    val pipes : MutableList<Pipe> = mutableListOf()
    lines.forEachIndexed { y, s ->
        lines[y].toCharArray().forEachIndexed { x, c ->
            if(c != '.'){
                pipes.add(Pipe(c,Pair(x,y)))
            }
        }
...
fun isConnectable(pipeA: Pipe, pipeB: Pipe, direction: String): Boolean {
    /*
...
LJ...
    ...
    if(pipeA.type == '|' && (direction == "north" )) return charArrayOf('S','|', '7', 'F').contains(pipeB.type)
    if(pipeA.type == '|' && (direction == "south" )) return charArrayOf('S','|', 'L', 'J').contains(pipeB.type)

    if(pipeA.type == '-' && (direction == "north" || direction == "south")) return false
    if(pipeA.type == '-' && (direction == "west" )) return charArrayOf('S','-', 'L', 'F').contains(pipeB.type)
    if(pipeA.type == '-' && (direction == "east" )) return charArrayOf('S','-', '7', 'J').contains(pipeB.type)

    if(pipeA.type == 'L' && (direction == "north")) return charArrayOf('S','|', '7', 'F').contains(pipeB.type)
    if(pipeA.type == 'L' && (direction == "south" || direction == "west")) return false
    if(pipeA.type == 'L' && (direction == "east" )) return charArrayOf('S','-', 'J', '7').contains(pipeB.type)

    if(pipeA.type == 'J' && (direction == "north")) return charArrayOf('S','|','7', 'F').contains(pipeB.type)
    if(pipeA.type == 'J' && (direction == "south" || direction == "east")) return false
    if(pipeA.type == 'J' && (direction == "west" )) return charArrayOf('S','-', 'F', 'L').contains(pipeB.type)

    if(pipeA.type == '7' && (direction == "north")) return false
    if(pipeA.type == '7' && (direction == "south" )) return charArrayOf('S','|', 'L', 'J').contains(pipeB.type)
    if(pipeA.type == '7' && (direction == "west" )) return charArrayOf('S','-', 'L', 'F').contains(pipeB.type)
    if(pipeA.type == '7' && (direction == "east" )) return false

    if(pipeA.type == 'F' && (direction == "north")) return false
    if(pipeA.type == 'F' && (direction == "south" )) return charArrayOf('S','|', 'L', 'J').contains(pipeB.type)
    if(pipeA.type == 'F' && (direction == "east" )) return charArrayOf('S','-', '7', 'J').contains(pipeB.type)
    if(pipeA.type == 'F' && (direction == "west" )) return false

    if(pipeA.type == 'S' && (direction == "north"))  return charArrayOf('|', '7', 'F').contains(pipeB.type)
    if(pipeA.type == 'S' && (direction == "south" )) return charArrayOf('|', 'L', 'J').contains(pipeB.type)
    if(pipeA.type == 'S' && (direction == "east" )) return charArrayOf('-').contains(pipeB.type)
    if(pipeA.type == 'S' && (direction == "west" )) return charArrayOf('-').contains(pipeB.type)
